...
def Q_2x2(scramble):
    ...
    U = 1/2*(basis(Nmax,0)@basis(Nmax,0).T + basis(Nmax,1)@basis(Nmax,1).T) \
        + basis(Nmax,5)@basis(Nmax,2).T + basis(Nmax,4)@basis(Nmax,3).T
    U = U + U.T
    
    D = 1/2*(basis(Nmax,0)@basis(Nmax,0).T + basis(Nmax,1)@basis(Nmax,1).T) \
        + basis(Nmax,5)@basis(Nmax,3).T + basis(Nmax,4)@basis(Nmax,2).T
    D = D + D.T
    
    R = 1/2*(basis(Nmax,2)@basis(Nmax,2).T + basis(Nmax,3)@basis(Nmax,3).T) \
        + basis(Nmax,5)@basis(Nmax,1).T + basis(Nmax,4)@basis(Nmax,0).T
    R = R + R.T
    
    L = 1/2*(basis(Nmax,2)@basis(Nmax,2).T + basis(Nmax,3)@basis(Nmax,3).T) \
        + basis(Nmax,5)@basis(Nmax,0).T + basis(Nmax,4)@basis(Nmax,1).T
    L = L + L.T
...
def M_2x2(scramble):
    ...
    for move in CS:
        ...
        MS.append((np.eye(Nmax) - 1j*move)/np.sqrt(2))
        MS.append(move)
    return [optimal_search(scramble,MS,Costs,maxdepth),scramble]

def Q_3x1(scramble):
    Costs = [1,1]
    maxdepth = 12
    Nmax = 4
    U = basis(Nmax,0)@basis(Nmax,1).T + 1/2*(basis(Nmax,2)@basis(Nmax,2).T+basis(Nmax,3)@basis(Nmax,3).T)
    U = U+U.T
    L = basis(Nmax,0)@basis(Nmax,3).T + 1/2*(basis(Nmax,2)@basis(Nmax,2).T+basis(Nmax,1)@basis(Nmax,1).T)
    L = L+L.T
    D = basis(Nmax,2)@basis(Nmax,3).T + 1/2*(basis(Nmax,1)@basis(Nmax,1).T+basis(Nmax,0)@basis(Nmax,0).T)
    D = D+D.T
    R = basis(Nmax,1)@basis(Nmax,2).T + 1/2*(basis(Nmax,0)@basis(Nmax,0).T+basis(Nmax,3)@basis(Nmax,3).T)
    R = R+R.T
...
def Q_1x1x1x1(scramble):
    Costs = [1,1]
    maxdepth = 10
    Nmax = 24
    U =   basis(Nmax,0)@basis(Nmax,6).T + basis(Nmax,1)@basis(Nmax,7).T\
        + basis(Nmax,2)@basis(Nmax,8).T + basis(Nmax,3)@basis(Nmax,9).T\
        + basis(Nmax,4)@basis(Nmax,10).T + basis(Nmax,5)@basis(Nmax,11).T\
        + basis(Nmax,12)@basis(Nmax,15).T + basis(Nmax,13)@basis(Nmax,14).T\
        + basis(Nmax,16)@basis(Nmax,17).T+ basis(Nmax,22)@basis(Nmax,23).T\
        + basis(Nmax,18)@basis(Nmax,20).T + basis(Nmax,19)@basis(Nmax,21).T
         
    U = U + U.T
    
    D = basis(Nmax,0)@basis(Nmax,1).T + basis(Nmax,6)@basis(Nmax,7).T\
        + basis(Nmax,2)@basis(Nmax,4).T + basis(Nmax,3)@basis(Nmax,5).T\
        + basis(Nmax,8)@basis(Nmax,10).T + basis(Nmax,9)@basis(Nmax,11).T\
        + basis(Nmax,12)@basis(Nmax,19).T + basis(Nmax,13)@basis(Nmax,18).T\
        + basis(Nmax,14)@basis(Nmax,20).T+ basis(Nmax,15)@basis(Nmax,21).T\
        + basis(Nmax,16)@basis(Nmax,22).T + basis(Nmax,17)@basis(Nmax,23).T
    D = D + D.T
    
    R =   basis(Nmax,4)@basis(Nmax,5).T + basis(Nmax,18)@basis(Nmax,19).T\
        + basis(Nmax,2)@basis(Nmax,0).T + basis(Nmax,3)@basis(Nmax,1).T\
        + basis(Nmax,6)@basis(Nmax,13).T + basis(Nmax,7)@basis(Nmax,12).T\
        + basis(Nmax,10)@basis(Nmax,16).T + basis(Nmax,11)@basis(Nmax,17).T\
        + basis(Nmax,9)@basis(Nmax,15).T+ basis(Nmax,8)@basis(Nmax,14).T\
        + basis(Nmax,20)@basis(Nmax,22).T + basis(Nmax,21)@basis(Nmax,23).T
    R = R + R.T
    
    L =   basis(Nmax,0)@basis(Nmax,21).T + basis(Nmax,1)@basis(Nmax,20).T\
        + basis(Nmax,2)@basis(Nmax,23).T + basis(Nmax,3)@basis(Nmax,22).T\
        + basis(Nmax,4)@basis(Nmax,18).T + basis(Nmax,5)@basis(Nmax,19).T\
        + basis(Nmax,6)@basis(Nmax,11).T + basis(Nmax,7)@basis(Nmax,10).T\
        + basis(Nmax,8)@basis(Nmax,9).T+ basis(Nmax,14)@basis(Nmax,15).T\
        + basis(Nmax,12)@basis(Nmax,16).T + basis(Nmax,13)@basis(Nmax,17).T
    L = L + L.T
